...
function Header({ label, field, config, sortBy, sortOrder, onSort }: { label: string; field?: string; config?:ColumnConfig; sortBy: string; sortOrder: string; onSort: (s:string)=>void }) {
  return <th style={styleFor(config)}>{field ? <button className="sort-button" onClick={() => onSort(field)}>{label}{sortBy === field ? (sortOrder === 'asc' ? <ArrowUp/> : <ArrowDown/>) : <ArrowUpDown/>}</button> : label}</th>
}
...
function SortableRow({item,kind,configs,submissionSteps,feedbackReviewers,feedbackStatusLabels,onView,onEdit,onAdvance,onDuplicate,onToggleAbandoned,onToggleTerminate,onDelete}:{item:Package;kind:PageKind;configs:ColumnConfig[];submissionSteps:Props['submissionSteps'];feedbackReviewers:Props['feedbackReviewers'];feedbackStatusLabels:Props['feedbackStatusLabels'];onView:(p:Package)=>void;onEdit:(p:Package)=>void;onAdvance:Props['onAdvance'];onDuplicate:Props['onDuplicate'];onToggleAbandoned:Props['onToggleAbandoned'];onToggleTerminate:Props['onToggleTerminate'];onDelete:Props['onDelete']}) {
  ...
  return <tr ref={setNodeRef} style={style} className={`${isDragging?'dragging':''} ${item.has_attachment?'has-attachment':''} ${item.is_abandoned?'abandoned':''}`} onDoubleClick={() => onView(item)}>
    ...
    {shown('document_type')&&<td style={styleFor(config('document_type'))}><StatusBadge status={item.document_type}/></td>}
    ...
    {shown('feedback')&&<td className="progress-cell feedback" style={styleFor(config('feedback'))}><FeedbackStatus item={item} reviewers={feedbackReviewers} statusLabels={feedbackStatusLabels} compact/></td>}
...
export function PackageTable({items,kind,configs,submissionSteps,feedbackReviewers,feedbackStatusLabels,sortBy,sortOrder,onSort,onView,onEdit,onReorder,onAdvance,onDuplicate,onToggleAbandoned,onToggleTerminate,onDelete}:Props) {
  ...
  const configuredWidth=kind==='documents'?configs.filter(item=>item.is_visible).reduce((total,item)=>total+item.column_width,110):undefined
  return <div className="table-scroll"><DndContext sensors={sensors} onDragEnd={dragEnd}><table className={`package-table ${kind==='documents'?'configurable-table':''}`} style={configuredWidth?{width:configuredWidth,minWidth:'100%'}:undefined}><thead><tr>
    <th className="drag-cell"/>{shown('document_number')&&<Header label={label('document_number',primary[0])} field={primary[1]} config={config(kind==='workflow'?'workflow_number':kind==='transmittal'?'transmittal_number':'document_number')} {...{sortBy,sortOrder,onSort}}/>}{kind === 'documents' && shown('document_title') && <Header label={label('document_title','Document Title')} field="document_title" config={config('document_title')} {...{sortBy,sortOrder,onSort}}/>}{shown('document_date')&&<Header label={label('document_date','Date')} field="document_date" config={config('document_date')} {...{sortBy,sortOrder,onSort}}/>}{kind==='workflow'&&<Header label="Terminate Workflow" field="workflow_terminated" {...{sortBy,sortOrder,onSort}}/>}{shown('document_type')&&<Header label={label('document_type','Document Type')} field="document_type" config={config('document_type')} {...{sortBy,sortOrder,onSort}}/>}{shown('initiator')&&<Header label={label('initiator','Initiator')} field="initiator" config={config('initiator')} {...{sortBy,sortOrder,onSort}}/>}{shown('discipline')&&<Header label={label('discipline','Discipline')} field="discipline" config={config('discipline')} {...{sortBy,sortOrder,onSort}}/>}{shown('number_of_documents')&&<Header label={label('number_of_documents','Docs')} field="number_of_documents" config={config('number_of_documents')} {...{sortBy,sortOrder,onSort}}/>}
    {kind === 'documents' && <>{shown('transmittal_number')&&<Header label={label('transmittal_number','Transmittal No.')} config={config('transmittal_number')} {...{sortBy,sortOrder,onSort}}/>}{shown('workflow_number')&&<Header label={label('workflow_number','Workflow No.')} config={config('workflow_number')} {...{sortBy,sortOrder,onSort}}/>}{shown('submission_progress')&&<Header label={label('submission_progress','Submission Progress')} config={config('submission_progress')} {...{sortBy,sortOrder,onSort}}/>}</>}{shown('feedback')&&<Header label={label('feedback','Feedback')} config={config('feedback')} {...{sortBy,sortOrder,onSort}}/>}<th/>
  </tr></thead><SortableContext items={ids} strategy={verticalListSortingStrategy}><tbody>{items.map(item => <SortableRow key={item.id} {...{item,kind,configs,submissionSteps,feedbackReviewers,feedbackStatusLabels,onView,onEdit,onAdvance,onDuplicate,onToggleAbandoned,onToggleTerminate,onDelete}}/>)}</tbody></SortableContext></table></DndContext></div>
}
