...
export function PackageTable({items,kind,configs,submissionSteps,feedbackReviewers,feedbackStatusLabels,feedbackStatusColors,sortBy,sortOrder,onSort,onColumnResize,onView,onEdit,onReorder,onAdvance,onDuplicate,onToggleAbandoned,onToggleTerminate,onDelete}:Props) {
  ...
  const configuredWidth=layoutConfigs.filter(item=>kind==='documents'?item.is_visible:kind==='workflow'?item.is_visible_workflow:item.is_visible_transmittal).reduce((total,item)=>total+item.column_width,kind==='workflow'?250:110)
  return <div className="table-scroll"><DndContext sensors={sensors} onDragEnd={dragEnd}><table className="package-table configurable-table" style={{width:configuredWidth,minWidth:'100%'}}><thead><tr>
    <th className="drag-cell"/>{shown(primaryField)&&<Header label={label(primaryField,primary[0])} field={primaryField} config={config(primaryField)} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{kind!=='documents'&&shown('document_number')&&<Header label={label('document_number','Document Number')} field="document_number" config={config('document_number')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('document_title') && <Header label={label('document_title','Document Title')} field="document_title" config={config('document_title')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('document_date')&&<Header label={label('document_date','Date')} field="document_date" config={config('document_date')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{kind==='workflow'&&<Header label="Terminate Workflow" field="workflow_terminated" {...{sortBy,sortOrder,onSort}}/>}{shown('document_type')&&<Header label={label('document_type','Document Type')} field="document_type" config={config('document_type')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('initiator')&&<Header label={label('initiator','Initiator')} field="initiator" config={config('initiator')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('discipline')&&<Header label={label('discipline','Discipline')} field="discipline" config={config('discipline')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('number_of_documents')&&<Header label={label('number_of_documents','Docs')} field="number_of_documents" config={config('number_of_documents')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}
