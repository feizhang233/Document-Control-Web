...
function SortableRow({item,highlighted,kind,configs,submissionSteps,feedbackReviewers,feedbackStatusLabels,feedbackStatusColors,onView,onEdit,onAdvance,onDuplicate,onToggleAbandoned,onToggleTerminate,onDelete}:{item:Package;highlighted:boolean;kind:PageKind;configs:ColumnConfig[];submissionSteps:Props['submissionSteps'];feedbackReviewers:Props['feedbackReviewers'];feedbackStatusLabels:Props['feedbackStatusLabels'];feedbackStatusColors:Props['feedbackStatusColors'];onView:(p:Package)=>void;onEdit:(p:Package)=>void;onAdvance:Props['onAdvance'];onDuplicate:Props['onDuplicate'];onToggleAbandoned:Props['onToggleAbandoned'];onToggleTerminate:Props['onToggleTerminate'];onDelete:Props['onDelete']}) {
  ...
  const shown=(field:ColumnField)=>{const item=config(field);return kind==='documents'?item?.is_visible!==false:kind==='workflow'?item?.is_visible_workflow!==false:item?.is_visible_transmittal!==false}
  return <tr ref={setNodeRef} data-package-id={item.id} style={style} className={`${isDragging?'dragging':''} ${item.has_attachment?'has-attachment':''} ${item.is_abandoned?'abandoned':''} ${highlighted?'notification-highlight':''}`} onDoubleClick={() => onView(item)}>
...
export function PackageTable({items,highlightedPackageId,kind,configs,submissionSteps,feedbackReviewers,feedbackStatusLabels,feedbackStatusColors,sortBy,sortOrder,onSort,onColumnResize,onView,onEdit,onReorder,onAdvance,onDuplicate,onToggleAbandoned,onToggleTerminate,onDelete}:Props) {
  ...
  const configuredWidth=layoutConfigs.filter(item=>kind==='documents'?item.is_visible:kind==='workflow'?item.is_visible_workflow:item.is_visible_transmittal).reduce((total,item)=>total+item.column_width,110)
  return <div className="table-scroll"><DndContext sensors={sensors} onDragEnd={dragEnd}><table className="package-table configurable-table" style={{width:configuredWidth,minWidth:'100%'}}><thead><tr>
    <th className="drag-cell"/>{shown(primaryField)&&<Header label={label(primaryField,primary[0])} field={primaryField} config={config(primaryField)} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{kind!=='documents'&&shown('document_number')&&<Header label={label('document_number','Document Number')} field="document_number" config={config('document_number')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('document_title') && <Header label={label('document_title','Document Title')} field="document_title" config={config('document_title')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('document_date')&&<Header label={label('document_date','Date')} field="document_date" config={config('document_date')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('document_type')&&<Header label={label('document_type','Document Type')} field="document_type" config={config('document_type')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('initiator')&&<Header label={label('initiator','Initiator')} field="initiator" config={config('initiator')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('discipline')&&<Header label={label('discipline','Discipline')} field="discipline" config={config('discipline')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('number_of_documents')&&<Header label={label('number_of_documents','Docs')} field="number_of_documents" config={config('number_of_documents')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}
    {primaryField!=='transmittal_number'&&shown('transmittal_number')&&<Header label={label('transmittal_number','Transmittal No.')} field="transmittal_number" config={config('transmittal_number')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{primaryField!=='workflow_number'&&shown('workflow_number')&&<Header label={label('workflow_number','Workflow No.')} field="workflow_number" config={config('workflow_number')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('submission_progress')&&<Header label={label('submission_progress','Submission Progress')} config={config('submission_progress')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}{shown('feedback')&&<Header label={label('feedback','Feedback')} config={config('feedback')} onResize={resize} {...{sortBy,sortOrder,onSort}}/>}<th/>
  </tr></thead><SortableContext items={ids} strategy={verticalListSortingStrategy}><tbody>{items.map(item => <SortableRow key={item.id} highlighted={item.id===highlightedPackageId} {...{item,kind,configs:layoutConfigs,submissionSteps,feedbackReviewers,feedbackStatusLabels,feedbackStatusColors,onView,onEdit,onAdvance,onDuplicate,onToggleAbandoned,onToggleTerminate,onDelete}}/>)}</tbody></SortableContext></table></DndContext></div>
}
