...
export function SettingsPage() {
  ...
  return <><div className="page-header"><div><div className="breadcrumb">Document Control <span>/</span> Settings</div><h1>Settings</h1><p>Control data backups and how document metadata is entered.</p></div></div>
    <div className="settings-layout settings-navigation-layout">
      ...
      {section==='workflow'&&<section className="settings-panel wide"><div className="settings-heading icon-heading"><span><Workflow/></span><div><h2>Workflow structure</h2><p>Edit the six Submission Progress stages, their order, the two Feedback reviewers and status labels.</p></div></div>
...
function WorkflowConfigEditor({config,onSaved}:{config:WorkflowConfig;onSaved:()=>void}){
  ...
  useEffect(()=>{setSteps([...config.submission_steps]);setReviewers([...config.feedback_reviewers]);setLabels({...config.feedback_status_labels});setColors({...config.feedback_status_colors})},[config])
  ...
  const save=useMutation({mutationFn:()=>settingsApi.updateWorkflow({submission_steps:steps,feedback_reviewers:reviewers,feedback_status_labels:labels,feedback_status_colors:colors}),onSuccess:()=>{toast.success('Workflow structure updated');onSaved()},onError:e=>toast.error(getApiError(e))})
