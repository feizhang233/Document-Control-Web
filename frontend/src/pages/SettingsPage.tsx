...
function WorkflowConfigEditor({config,onSaved}:{config:WorkflowConfig;onSaved:()=>void}){
  ...
  return <div className="workflow-config-editor">
    <div className="workflow-config-block"><div className="workflow-config-title"><div><strong>Submission Progress</strong><span>Exactly five stages. Use arrows to change the sequence.</span></div><small>5 stages</small></div><div className="workflow-step-list">{steps.map((step,index)=><div className="workflow-step-row" key={index}><b>{index+1}</b><input aria-label={`Submission step ${index+1}`} value={step} onChange={event=>setSteps(previous=>previous.map((value,i)=>i===index?event.target.value:value))}/><button type="button" disabled={index===0} onClick={()=>move(index,-1)} aria-label="Move stage up"><ArrowUp/></button><button type="button" disabled={index===steps.length-1} onClick={()=>move(index,1)} aria-label="Move stage down"><ArrowDown/></button></div>)}</div></div>
