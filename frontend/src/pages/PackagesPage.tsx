...
import { feedbackSteps, submissionSteps, type FilterRule, type Package, type PackageInput, type PageKind, type Period } from '../types/package'
...
export function PackagesPage({ kind }: { kind: PageKind }) {
  ...
  const visibleItems=useMemo(()=>{
    ...
    const valueFor=(item:Package,field:FilterRule['field']):string=>{
      ...
      if(field==='feedback')return item.feedback.Terminate?'terminated':String(Math.round(feedbackSteps.filter(step=>step!=='Terminate'&&item.feedback[step]).length/2*100))
  ...
  const advance=(item:Package,type:'submission'|'feedback')=>{
    if(type==='submission'){
      const next=submissionSteps.find(step=>!item.submission_progress[step]);if(!next)return
      quickUpdate.mutate({id:item.id,data:{submission_progress:{...item.submission_progress,[next]:true}}});return
    }
    const next=feedbackSteps.find(step=>!item.feedback[step]);if(!next)return
  ...
  return <>
    ...
    <section className="data-card">
      ...
      {query.isLoading ? <LoadingState/> : query.isError ? <ErrorState message={getApiError(query.error)} retry={()=>query.refetch()}/> : !visibleItems.length ? <EmptyState filtered={!!search || !!discipline || !!filters.length}/> : <PackageTable items={visibleItems} kind={kind} {...{sortBy,sortOrder}} onSort={key => { if(sortBy===key) setSortOrder(v=>v==='asc'?'desc':'asc'); else {setSortBy(key);setSortOrder('asc')} }} onView={setSelected} onEdit={item=>{setEditing(item);setEditorOpen(true)}} onReorder={ids=>reorder.mutate(ids)} onAdvance={advance} onDuplicate={item=>duplicate.mutate(item)} onToggleAbandoned={item=>quickUpdate.mutate({id:item.id,data:{is_abandoned:!item.is_abandoned}})} onToggleTerminate={item=>quickUpdate.mutate({id:item.id,data:{workflow_terminated:!item.workflow_terminated}})}/>} 
    ...
    <PackageDrawer item={selected} saving={quickUpdate.isPending} onUpdate={data=>selected&&quickUpdate.mutate({id:selected.id,data})} onClose={()=>setSelected(null)}/>
    <PackageEditor item={editing} configs={configs.data||[]} open={editorOpen} saving={save.isPending} onClose={()=>setEditorOpen(false)} onSave={data=>save.mutate(data)}/>
  </>
}
