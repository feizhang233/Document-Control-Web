...
export function DashboardPage() {
  ...
  const completionRate=items.length?Math.round(complete/items.length*100):0
  ...
  return <>
    ...
    <div className="dashboard-row dashboard-row-secondary">
      ...
      <section className="panel dashboard-panel overview-panel"><div className="panel-heading"><div><h2>Workflow overview</h2><p>Current submission health</p></div><span className="trend"><TrendingUp size={14}/> Live</span></div><div className="donut-wrap"><div className="donut" style={{'--progress':`${completionRate}%`} as React.CSSProperties}><div><strong>{completionRate}%</strong><span>complete</span></div></div><div className="donut-legend"><div><i className="blue"/><span>In progress</span><strong>{active}</strong></div><div><i className="green"/><span>Completed</span><strong>{complete}</strong></div><div><i className="amber"/><span>Feedback due</span><strong>{feedbackPending}</strong></div></div></div><Link className="panel-action" to="/workflow"><Send/> Review workflow register <ArrowRight/></Link></section>
    </div>
  </>
}
